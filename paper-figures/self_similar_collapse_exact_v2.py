#!/usr/bin/env python3
"""
self_similar_collapse_exact_v2.py - Plot self-similar collapse with exact theoretical scaling.

Plots two datasets (θ=10° and θ=20°) on the same figure with different colormaps.

Scaling (using θ⁴t instead of h₀):
    y-axis: h / (θ^4 * t)
    x-axis: x / (θ^3 * t)

Usage:
    python self_similar_collapse_exact_v2.py
"""
...
def main():
    # Configuration
    x_lim, h_lim = 1.0, 1.0
    output_file = 'self_similar_collapse_exact_v2.pdf'
    ...
    for data_dir, theta_deg, cmap, t_min, t_max, label in datasets:
        ...
        print(f"  θ³ = {theta**3:.6f}, θ⁴ = {theta**4:.6f}")
        ...
        for snap in snapshots:
            ...
            # Exact theoretical scaling using θ⁴t
            x_scale = theta**3 * t
            h_scale = theta**4 * t

            x_scaled = (snap['x'] - x0) / x_scale
            h_scaled = snap['h'] / h_scale
    ...
    # Axis labels
    ax.set_xlabel(r'$\displaystyle\frac{x}{\theta^3 t}$')
    ax.set_ylabel(r'$\displaystyle\frac{h(x,t)}{\theta^4 t}$')
