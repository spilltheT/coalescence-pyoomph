#!/usr/bin/env python3
"""
self_similar_collapse_exact.py - Plot exact self-similar collapse with theoretical scaling.

Plots two datasets (θ=10° and θ=20°) on the same figure with different colormaps.
...
Usage:
    python self_similar_collapse_exact.py
"""
...
def main():
    # Configuration
    x_lim, h_lim = 4.0, 4.0
    output_file = 'self_similar_collapse_exact.pdf'

    # Dataset configurations: (data_dir, theta_deg, colormap, t_min, t_max, label)
    datasets = [
        ('hx-Pe1-beta0p1-theta10', 10.0, cm.Blues, 4.0, 8.0, r'$\theta=10^\circ$'),
        ('hx-Pe1-beta0p1-theta20', 20.0, cm.hot_r, 1.0, 2.0, r'$\theta=20^\circ$'),
    ]
    ...
    for data_dir, theta_deg, cmap, t_min, t_max, label in datasets:
        ...
        print(f"  θ = {theta_deg}° = {theta:.6f} rad")
        ...
        for snap in snapshots:
            ...
            # Scale by h_0(t) with theta factor on x
            x_scaled = (snap['x'] - x0) * theta / h0
            h_scaled = snap['h'] / h0

            color = cmap(norm(t))
            ax.plot(x_scaled, h_scaled, '-',
                    linewidth=2, color=color, alpha=0.7)

    # Axis labels
    ax.set_xlabel(r'$\displaystyle\frac{x \theta}{h_0(t)}$')
    ...
    cbar1.set_label(r'$t$ ($\theta=20^\circ$)', fontsize=11)
    cbar1.ax.tick_params(labelsize=10)

    # Second colorbar (θ=10°, Blues, t=4-16)
    ...
    cbar2.set_label(r'$t$ ($\theta=10^\circ$)', fontsize=11)
