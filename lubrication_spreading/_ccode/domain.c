...
static void ResidualAndJacobian0(const JITElementInfo_t * eleminfo, const JITShapeInfo_t * shapeinfo,double * residuals, double *jacobian, double *mass_matrix,unsigned flag)
{
  ...
  for(unsigned ipt=0;ipt<shapeinfo->n_int_pt;ipt++)
  {
    ...
    {
      this_intrp_d0t0_d0x_coordinate_x+= eleminfo->nodal_coords[l_shape][this_nodalind_coordinate_x][0] * shapeinfo->shape_Pos[l_shape];
    }
 ...
 //Subexpressions // TODO: Check whether it is constant to take it out of the loop
    ...
    double _res_contrib,_J_contrib;
    {
      ...
      {
        BEGIN_RESIDUAL_CONTINUOUS_SPACE(eleminfo->nodal_local_eqn[l_test][this_nodalind_h],( 2.0943951023931957e+00*dx_testfunction[l_test][0]*this_intrp_d0t0_d0x_coordinate_x*this_intrp_d0t0_d1x0_p*pow(this_intrp_d0t0_d0x_h,3.0)+6.2831853071795862e+00*testfunction[l_test]*this_intrp_d0t0_d0x_coordinate_x*this_intrp_d1t0BDF2_d0x_h)*dx, shapeinfo->hanginfo_C2,this_nodalind_h,l_test)
          ADD_TO_RESIDUAL_CONTINUOUS_SPACE()
          BEGIN_JACOBIAN()
            for (unsigned int l_shape=0;l_shape<eleminfo->nnode_C2;l_shape++)
            {
              BEGIN_JACOBIAN_HANG(eleminfo->nodal_local_eqn[l_shape][this_nodalind_p], 2.0943951023931957e+00*dx_testfunction[l_test][0]*dx*this_intrp_d0t0_d0x_coordinate_x*shapeinfo->dx_shape_C2[l_shape][0]*pow(this_intrp_d0t0_d0x_h,3.0),shapeinfo->hanginfo_C2,this_nodalind_p,l_shape)
                ADD_TO_JACOBIAN_HANG_HANG()
              END_JACOBIAN_HANG()
              BEGIN_JACOBIAN_HANG(eleminfo->nodal_local_eqn[l_shape][this_nodalind_h], ( 6.2831853071795862e+00*testfunction[l_test]*this_intrp_d0t0_d0x_coordinate_x*shapeinfo->timestepper_weights_dt_BDF2_degr[0]*shapeinfo->shape_C2[l_shape]+6.2831853071795862e+00*dx_testfunction[l_test][0]*this_intrp_d0t0_d0x_coordinate_x*this_intrp_d0t0_d1x0_p*pow(this_intrp_d0t0_d0x_h,2.0)*shapeinfo->shape_C2[l_shape])*dx,shapeinfo->hanginfo_C2,this_nodalind_h,l_shape)
                ADD_TO_JACOBIAN_HANG_HANG()
                ADD_TO_MASS_MATRIX_HANG_HANG(6.2831853071795862e+00*testfunction[l_test]*dx*this_intrp_d0t0_d0x_coordinate_x*shapeinfo->shape_C2[l_shape])
              END_JACOBIAN_HANG()
            }
          END_JACOBIAN()
        END_RESIDUAL_CONTINUOUS_SPACE()
        BEGIN_RESIDUAL_CONTINUOUS_SPACE(eleminfo->nodal_local_eqn[l_test][this_nodalind_p],2.0000000000000000e+00*( -3.1415926535897931e+00*dx_testfunction[l_test][0]*this_intrp_d0t0_d0x_coordinate_x*this_intrp_d0t0_d1x0_h+3.1415926535897931e+00*testfunction[l_test]*( -1.4457428321908239e-05*( pow(this_intrp_d0t0_d0x_h,3.0)-4.2187499999999993e-07)/pow(this_intrp_d0t0_d0x_h,6.0)+this_intrp_d0t0_d0x_p)*this_intrp_d0t0_d0x_coordinate_x)*dx, shapeinfo->hanginfo_C2,this_nodalind_p,l_test)
          ADD_TO_RESIDUAL_CONTINUOUS_SPACE()
          BEGIN_JACOBIAN()
            for (unsigned int l_shape=0;l_shape<eleminfo->nnode_C2;l_shape++)
            {
              BEGIN_JACOBIAN_HANG(eleminfo->nodal_local_eqn[l_shape][this_nodalind_p], 6.2831853071795862e+00*testfunction[l_test]*dx*this_intrp_d0t0_d0x_coordinate_x*shapeinfo->shape_C2[l_shape],shapeinfo->hanginfo_C2,this_nodalind_p,l_shape)
                ADD_TO_JACOBIAN_HANG_HANG()
              END_JACOBIAN_HANG()
              BEGIN_JACOBIAN_HANG(eleminfo->nodal_local_eqn[l_shape][this_nodalind_h], 2.0000000000000000e+00*( -3.1415926535897931e+00*dx_testfunction[l_test][0]*this_intrp_d0t0_d0x_coordinate_x*shapeinfo->dx_shape_C2[l_shape][0]+3.1415926535897931e+00*( 8.6744569931449428e-05*( pow(this_intrp_d0t0_d0x_h,3.0)-4.2187499999999993e-07)/pow(this_intrp_d0t0_d0x_h,7.0)*shapeinfo->shape_C2[l_shape]+-4.3372284965724714e-05*1.0/pow(this_intrp_d0t0_d0x_h,4.0)*shapeinfo->shape_C2[l_shape])*testfunction[l_test]*this_intrp_d0t0_d0x_coordinate_x)*dx,shapeinfo->hanginfo_C2,this_nodalind_h,l_shape)
                ADD_TO_JACOBIAN_HANG_HANG()
              END_JACOBIAN_HANG()
            }
          END_JACOBIAN()
...
static double ElementalInitialConditions0(const JITElementInfo_t * eleminfo, int field_index,double *_x, double *_xlagr,double *_normal,double t,int flag,double default_val)
{
  ...
  {
    if (!flag) return 5.0000000000000000e-01*fmax(-(_x[0]*_x[0])+1.0000000000000000e+00, 1.4999999999999999e-02); 
...
static void JacobianForElementSizeSpatialDerivatives(const JITElementInfo_t * eleminfo, const double * _x,double *grad)
{
   grad[0] = 6.2831853071795862e+00;
}
...
JIT_API void JIT_ELEMENT_init(JITFuncSpec_Table_FiniteElement_t *functable)
{
 ...
 functable->nodal_dim=1;
 functable->lagr_dim=1;
 ...
 SET_INTERNAL_FIELD_NAME(functable->fieldnames_Pos,1, "lagrangian_x" );
 ...
 functable->temporal_error_scales=calloc(5,sizeof(double)); 
 functable->discontinuous_refinement_exponents=calloc(5,sizeof(double));
 functable->num_ICs=1;
 ...
 SET_INTERNAL_FIELD_NAME(functable->Dirichlet_names,1, "" ); // nodal_coords index is 1
 ...
 functable->JacobianForElementSizeSpatialDerivative=&JacobianForElementSizeSpatialDerivatives;
